...
SELECT email, state FROM sales.customers WHERE state = 'SC' or state = 'MG';

SELECT email, state FROM sales.customers WHERE state = 'SP' or state = 'MG';

-- Por algum motivo BA nao vai com outros estados em or

-- (Exemplo 2) Filtro com mais de uma condição
-- Liste os emails dos clientes da nossa base que moram no estado de Santa Catarina
-- ou Mato Grosso do Sul
-- OBRIGATORIO o uso de aspas simples
SELECT email, state FROM sales.customers WHERE state = 'SC';
SELECT email, state FROM sales.customers WHERE state = 'BA';

-- (Exemplo 3) Filtro de condição com data
-- Liste os emails dos clientes da nossa base que moram no estado de Santa Catarina
-- ou Mato Grosso do Sul e que tem mais de 30 anos

SELECT email, state, birth_date FROM sales.customers WHERE (state = 'SC' or state = 'MG') and birth_date > '1995-01-01'; -- podemos fazer em 19950101 sem formatação

SELECT birth_date FROM sales.customers;
